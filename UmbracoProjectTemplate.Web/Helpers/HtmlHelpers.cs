...
namespace UmbracoProjectTemplate.Web.Helpers
{
    ...
    public static class HtmlHelpers
    {
        ...
        /// <returns>A Html string to insert in the output</returns>
        public static MvcHtmlString LazyLoadHelper(this HtmlHelper helper, string mediaId, string crop,
            string mobileCrop, string title, string cssClass)
        {
            var umbHelper = new UmbracoHelper(UmbracoContext.Current);
            var image = null as IPublishedContent;
            try
            ...
            if (image != null)
            {
                var lazyload = new MvcHtmlString(string.Format(
                    ...
                    "<noscript><img src='{1}' class='img-responsive {3}' alt='{2}')' /></noscript>",
                    image.GetCropUrl(mobileCrop), image.GetCropUrl(crop), title, cssClass));
        ...
        /// <returns>A Html string to insert in the output</returns>
        public static MvcHtmlString LazyLoadHelper(this HtmlHelper helper, string mediaId, string crop,
            string mobileCrop, string title, string cssClass, string divClass)
        {
            var umbHelper = new UmbracoHelper(UmbracoContext.Current);
            var image = null as IPublishedContent;
            try
            ...
            if (image != null)
            {
                var lazyload = new MvcHtmlString(string.Format(
                    ...
                    "src='data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=='" +
                    "data-src-small='{0}' data-src='{1}' alt='{2}'/>" +
                    "<noscript><img src='{1}' class='img-responsive {3}' alt='{2}')' /></noscript></div>",
                    image.GetCropUrl(mobileCrop), image.GetCropUrl(crop), title, cssClass, divClass));
